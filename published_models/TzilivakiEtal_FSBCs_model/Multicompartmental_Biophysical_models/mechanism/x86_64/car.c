...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_prop_size(_mechtype, 15, 5);
 	...
 	ivoc_help("help ?1 car /home/saray/TzilivakiEtal_FSBCs_model/Multicompartmental_Biophysical_models/mechanism/x86_64/car.mod\n");
...
double varss ( _threadargsprotocomma_ double _lv , double _li ) {
   double _lvarss;
 if ( _li  == 0.0 ) {
     _lvarss = 1.0 / ( 1.0 + exp ( ( _lv + 43.5 ) / ( - 3.0 ) ) ) ;
     }
   else if ( _li  == 1.0 ) {
     _lvarss = 1.0 / ( 1.0 + exp ( ( _lv + 50.0 ) / ( 1.0 ) ) ) ;
     }
